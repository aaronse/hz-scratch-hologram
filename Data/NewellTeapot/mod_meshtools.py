# $Id: mod_meshtools.py,v 1.5 2004/11/30 02:27:46 ianwill Exp $
#
# +---------------------------------------------------------+
...
# =================================
def append_faces(mesh, faces, facesuv, uvcoords, facesvc, vcolors):
		...
		if numfaceverts <= 4:				# This face is a triangle or quad
			...
			for j in range(numfaceverts):
				...
				if uvcoords:
					...
					face.mode = 0
					#face.col = [Blender.NMesh.Col(0,128,255)]*4
					#vcidx = facesvc[i][j]
...
# === Create Blender Mesh ===
# ===========================
def create_mesh(verts, faces, objname, facesuv=[], uvcoords=[], normals=[], facesvc=[], vcolors=[]):
	if normals: normal_flag = 0
	else: normal_flag = 1
	mesh = Blender.NMesh.New()
	#mesh.setMode() # clear the TwoSided flag
	append_verts(mesh, verts, normals)
	append_faces(mesh, faces, facesuv, uvcoords, facesvc, vcolors)
...
# def reverse_list_of_lists(LOL):
#	  def Reverse(List):
#		  List.reverse()
#		  return List
#	  return map(Reverse, LOL)
#
# def reverse_list_of_lists(LOL):
#	  return [[C,B,A] for [A,B,C] in LOL]
