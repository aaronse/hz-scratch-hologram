...
namespace ScratchUtility
{
    ...
    public struct Coord
    {
        ...
        public bool Equals(object obj, int decimalPlaces)
        {
            if (obj.GetType() != typeof(Coord))
            {
                return false;
            ...
            // Return true if the fields match:
            return Math.Round(a.X, decimalPlaces) == Math.Round(b.X, decimalPlaces) &&
                Math.Round(a.Y, decimalPlaces) == Math.Round(b.Y, decimalPlaces) &&
                Math.Round(a.Z, decimalPlaces) == Math.Round(b.Z, decimalPlaces);
        }
